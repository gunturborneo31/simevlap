...
<template>
  <AppLayout title="Urusan">
    <section class="mb-8">
      <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4">
        <h1 class="text-2xl font-bold text-emerald-900">Daftar Urusan</h1>
        <div class="flex gap-2">
          <form @submit.prevent="searchUrusan" class="flex gap-2">
            <input v-model="search" type="text" placeholder="Cari urusan..." class="border-2 border-emerald-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-400" />
            <button type="submit" class="bg-emerald-600 text-white rounded-lg px-4 py-2 font-bold hover:bg-emerald-700">Cari</button>
          </form>
          <button @click="openAdd" class="bg-emerald-600 text-white rounded-lg px-4 py-2 font-bold hover:bg-emerald-700">+ Tambah Urusan</button>
        </div>
      </div>
        ...
        <table class="min-w-full text-sm">
          <thead class="bg-gray-50 border-b border-emerald-100">
            <tr>
              ...
              <th class="px-4 py-3 text-left font-bold text-gray-700">NAMA_URUSAN</th>
              <th class="px-4 py-3 text-left font-bold text-gray-700 w-32">Aksi</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="urusan in urusans.data" :key="urusan.id" class="hover:bg-gray-50 transition">
              <td class="px-4 py-2">{{ urusan.kode }}</td>
              <td class="px-4 py-2">{{ urusan.nama }}</td>
              <td class="px-4 py-2">
                <button @click="openEdit(urusan)" class="text-blue-600 hover:underline mr-2">Edit</button>
                <button @click="openDelete(urusan)" class="text-red-600 hover:underline">Hapus</button>
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <div class="mt-4 flex justify-center">
        <Pagination :links="urusans.links" />
      </div>
    </section>

    <!-- Modal Tambah/Edit -->
    <Modal :show="showForm" :title="formMode === 'add' ? 'Tambah Urusan' : 'Edit Urusan'" @close="closeForm">
    ...
    <Modal :show="showDelete" title="Konfirmasi Hapus" @close="closeDelete">
      <div class="mb-4">Yakin ingin menghapus urusan <b>{{ form.nama }}</b>?</div>
...
function searchUrusan() {
  router.get(route('urusan.index'), { search: search.value }, { preserveState: true, replace: true });
}
...
function openEdit(urusan) {
  formMode.value = 'edit';
  form.id = urusan.id;
  form.kode = urusan.kode;
  form.nama = urusan.nama;
  errors.kode = '';
  errors.nama = '';
  showForm.value = true;
}
...
function submitForm() {
  ...
  if (formMode.value === 'add') {
    router.post(route('urusan.store'), { kode: form.kode, nama: form.nama }, {
      preserveScroll: true,
      onError: (err) => {
    ...
    router.put(route('urusan.update', form.id), { kode: form.kode, nama: form.nama }, {
      preserveScroll: true,
      onError: (err) => {
        errors.kode = err.kode;
        errors.nama = err.nama;
...
function submitDelete() {
  router.delete(route('urusan.destroy', form.id), {
    preserveScroll: true,
    onSuccess: () => {
      showDelete.value = false;
    }
